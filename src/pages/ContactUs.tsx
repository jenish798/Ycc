...
const MEAL_TYPES = [
  { id: "breakfast", name: "Breakfast", timeSlots: ["7:00 AM - 8:00 AM", "8:00 AM - 9:00 AM", "9:00 AM - 10:00 AM"] },
  { id: "lunch", name: "Lunch", timeSlots: ["12:00 PM - 1:00 PM", "1:00 PM - 2:00 PM", "2:00 PM - 3:00 PM"] },
  { id: "dinner", name: "Dinner", timeSlots: ["7:00 PM - 8:00 PM", "8:00 PM - 9:00 PM", "9:00 PM - 10:00 PM"] },
]

// Menu items for each meal type
const MENU_ITEMS = {
  breakfast: [
    { id: "idli", name: "Idli with Sambar & Chutney", price: 120, popular: true },
    { id: "dosa", name: "Masala Dosa with Sambar & Chutney", price: 150, popular: false },
    { id: "vada", name: "Vada with Sambar & Chutney", price: 100, popular: false },
    { id: "pongal", name: "Pongal with Sambar & Chutney", price: 130, popular: true },
    { id: "upma", name: "Upma with Coconut Chutney", price: 110, popular: false },
  ],
  lunch: [
    { id: "meals", name: "South Indian Thali (Full Meals)", price: 250, popular: true },
    { id: "biryani", name: "Vegetable Biryani", price: 200, popular: true },
    { id: "curdRice", name: "Curd Rice with Pickle", price: 120, popular: false },
    { id: "lemonRice", name: "Lemon Rice with Papad", price: 130, popular: false },
    { id: "sambarRice", name: "Sambar Rice with Papad", price: 140, popular: false },
  ],
  dinner: [
    { id: "chapati", name: "Chapati with Kurma", price: 150, popular: false },
    { id: "parotta", name: "Parotta with Salna", price: 160, popular: true },
    { id: "idiyappam", name: "Idiyappam with Kurma", price: 140, popular: false },
    { id: "appam", name: "Appam with Coconut Milk", price: 150, popular: true },
    { id: "dinnerThali", name: "Mini Dinner Thali", price: 200, popular: false },
  ],
}

export default function OrderBooking() {
  const navigate = useNavigate()
  const [orderType, setOrderType] = useState<"oneTime" | "subscription">("oneTime")
  ...
  const handleQuantityChange = (mealType: string, itemId: string, quantity: number) => {
  ...
  const isFormValid = () => {
    ...
    if (orderType === "subscription") {
      return hasItems && hasAddress && subscriptionDays.length > 0 && hasMealType;
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative w-full h-[300px] overflow-hidden">
        ...
        <img
          src="https://source.unsplash.com/random/1200x300/?indian-food"
          alt="Order Food"
        ...
        <div className="relative z-20 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
          ...
          <p className="text-lg text-turmeric-50 max-w-2xl">
            Order authentic South Indian cuisine for one-time delivery or daily subscription
          </p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <FadeIn>
                  <Card className="border-turmeric-200 mb-6">
                    <CardContent className="p-6">
                      <h2 className="text-xl font-bold text-purple-800 mb-4">Order Type</h2>
                      <RadioGroup
                        value={orderType}
                        onValueChange={(value) => setOrderType(value as "oneTime" | "subscription")}
                        className="space-y-3"
                      >
                        <div className="flex items-start space-x-2">
                          <RadioGroupItem value="oneTime" id="oneTime" className="mt-1" />
                          <div>
                            <Label htmlFor="oneTime" className="text-base font-medium">
                              One-Time Order
                            </Label>
                            <p className="text-sm text-turmeric-700">Order for a specific date and time slot</p>
                          </div>
                        </div>
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-turmeric-700">Regular delivery on selected days of the week</p>
                          </div>
                        </div>
                      </RadioGroup>
                    </CardContent>
                  ...
                  {orderType === "oneTime" ? (
                    <Card className="border-turmeric-200 mb-6">
                      <CardContent className="p-6">
                        <h2 className="text-xl font-bold text-purple-800 mb-4">Schedule</h2>
                                ...
                                <div key={meal.id} className="flex items-center space-x-2">
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      if (checked) {
                                        setSelectedMealType((prev) => [...prev, meal.id]);
                                      } else {
                                        setSelectedMealType((prev) => prev.filter((id) => id !== meal.id));
                                      }
                                    }}
                                  />
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center">
                              ...
                              <Select value={selectedTimeSlot} onValueChange={setSelectedTimeSlot}>
                                ...
                                <SelectContent>
                                  {MEAL_TYPES.find((meal) => selectedMealType.includes(meal.id))?.timeSlots?.map((slot) => (
                    ...
                    <Card className="border-turmeric-200 mb-6">
                      <CardContent className="p-6">
                        <h2 className="text-xl font-bold text-purple-800 mb-4">Subscription Details</h2>
                        <div className="space-y-4">
                          <div className="space-y-2">
                  ...
                  <Card className="border-turmeric-200">
                    <CardContent className="p-6">
                      <h2 className="text-xl font-bold text-purple-800 mb-4">Delivery Details</h2>
                      <div className="space-y-4">
                        <div className="space-y-2">
              ...
              <div className="lg:col-span-1">
                <FadeIn direction="up">
                  <Card className="border-turmeric-200">
                    <CardContent className="p-6">
                      <h2 className="text-xl font-bold text-purple-800 mb-4">Menu Selection</h2>
                      {selectedMealType.length === 0 ? (
                        <div className="text-center py-8">
                          <p className="text-turmeric-700">Please select at least one meal type</p>
                        </div>
                      ) : (
                          ...
                          {selectedMealType.map((mealType) => (
                            <div key={mealType} className="space-y-4">
                              <h3 className="font-medium text-purple-700 border-b border-turmeric-200 pb-2">
                                {mealType === "breakfast" ? "Breakfast Menu" : mealType === "lunch" ? "Lunch Menu" : "Dinner Menu"}
                              </h3>
                              {MENU_ITEMS[mealType as keyof typeof MENU_ITEMS].map((item) => (
                                <div
                                ...
                                >
                                  <div className="flex-1">
                                    ...
                                    <p className="text-turmeric-700">₹{item.price}</p>
                                  </div>
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleQuantityChange(
                                          mealType,
                                          item.id,
                                          Math.max(0, (selectedItems[`${mealType}-${item.id}`] || 0) - 1)
                                        )
                                      }
                                    >
                                      -
                                    </Button>
                                    <span className="w-8 text-center">{selectedItems[`${mealType}-${item.id}`] || 0}</span>
                                    <Button
                                      type="button"
                                      variant="outline"
                                      size="sm"
                                      ...
                                      onClick={() =>
                                        handleQuantityChange(
                                          mealType,
                                          item.id,
                                          (selectedItems[`${mealType}-${item.id}`] || 0) + 1
                                        )
                                      }
                                    >
                                      +
                ...
                <FadeIn direction="left">
                  <Card className="border-turmeric-200 mb-6 sticky top-20">
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          <h3 className="font-medium text-purple-700 mb-2">Selected Items</h3>
                          {Object.entries(selectedItems).length > 0 ? (
                            <div className="space-y-4">
                              {selectedMealType.map((mealType) => {
                                const mealItems = Object.entries(selectedItems)
                                  .filter(([key, quantity]) => key.startsWith(`${mealType}-`) && quantity > 0)
                                  .map(([key, quantity]) => {
                                    const itemId = key.split('-')[1];
                                    return {
                                      id: itemId,
                                      name: MENU_ITEMS[mealType as keyof typeof MENU_ITEMS].find((item) => item.id === itemId)?.name || '',
                                      price: MENU_ITEMS[mealType as keyof typeof MENU_ITEMS].find((item) => item.id === itemId)?.price || 0,
                                      quantity,
                                    };
                                  });
                                ...
                                return (
                                  <div key={mealType} className="space-y-2">
                                    ...
                                    {mealItems.map((item) => (
                                      <div key={`${mealType}-${item.id}`} className="flex justify-between">
                                        <div className="flex items-center">
                                          <span className="text-turmeric-800">{item.name}</span>
                                          <span className="text-turmeric-600 ml-2">x{item.quantity}</span>
                                        </div>
                            ...
                            <p className="text-turmeric-600 italic">No items selected</p>
                          )}
                        </div>
                        ...
                        <div className="border-t border-turmeric-200 pt-4">
                          <h3 className="font-medium text-purple-700 mb-2">Payment Method</h3>
                          ...
                          <div className="bg-purple-50 p-3 rounded-md">
                            <div className="flex items-start">
                              ...
                              <p className="text-sm text-purple-700">
                                Subscription orders will be delivered on your selected days at the chosen time slot.
                                Payment will be collected weekly.
                              </p>
                            </div>
                          </div>
                        )}
                        ...
                        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                          ...
                          >
                            {orderType === "oneTime" ? "Place Order" : "Start Subscription"}
                          </Button>
                        </motion.div>
                      </div>
                    </CardContent>
